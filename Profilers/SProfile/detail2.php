...
FusionCharts.ready(function() {
  ...
  <?php
			...
			$check = mysqli_query($connect, "SELECT * FROM crt_marks WHERE S_id='$usn' and sub_id='quant011'") or die("Check Query");
			$row = mysqli_fetch_assoc($check);
			$best = $row['marks'];
			
				$check1 = mysqli_query($connect, "SELECT Test_date,marks FROM crt_marks WHERE Test_date=
				(SELECT max(Test_date) from crt_marks WHERE S_id='65402154') and sub_id='quant011' ") or die("Check Query");
  ...
  chart: {
   ...
   <?php
			$connect = mysqli_connect("localhost", "root", "");
			mysqli_select_db($connect, "spsp") or die("Cant Connect to database");
			$check = mysqli_query($connect, "SELECT max(marks) p FROM crt_marks WHERE S_id='$usn' and sub_id='pi011'") or die("Check Query");
			$row = mysqli_fetch_assoc($check);
			$best = $row['p'];
			$check1 = mysqli_query($connect, "SELECT marks FROM crt_marks WHERE Test_date=
