...
mysqli_select_db($connect, "spsp"); // Selecting Database from Server
	...
	if($Password && $repassword && $cur) 
	{
		...
		{
			$sql = mysqli_query($connect, "SELECT * FROM `spsp`.`stu_login` WHERE `S_id`='$Username'");
			if(mysqli_num_rows($sql) == 1)
			{
				...
				if($cur == $dbpassword)
				{
					if($query = mysqli_query($connect,"UPDATE `spsp`.`stu_login` SET `password` = '$Password' WHERE `stu_login`.`S_id` = '$Username'"))
					{
