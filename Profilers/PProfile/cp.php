...
<html lang="en">
  <head>
    ...
    <title>TPO - Change Password</title>
...
mysqli_select_db($connect, "spsp"); // Selecting Database from Server
	...
	if($Password && $repassword && $cur) 
	{
		...
		{
			$sql = mysqli_query($connect, "SELECT * FROM `spsp`.`tpo_detail` WHERE `tpo_id`='$Username'");
			if(mysqli_num_rows($sql) == 1)
			{
				...
				if($cur == $dbpassword)
				{
					if($query = mysqli_query($connect, "UPDATE `spsp`.`tpo_detail` SET `password` = '$Password' WHERE `tpo_detail`.`tpo_id` = '$Username'"))
					{
