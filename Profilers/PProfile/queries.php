...
$RESULT=mysqli_query($connect, "SELECT DISTINCT count(com_id) from company_detail where Venue LIKE '%UCER%' AND YEAR(Date)=YEAR(NOW())");
$data=mysqli_fetch_assoc($RESULT);
echo "<br><br><h3>Companies In Our Campus In This Year&nbsp:&nbsp";
echo $data['count(com_id)'];
$RESULT=mysqli_query($connect, "SELECT count(Attendance) from placed_students where Attendance=1 AND YEAR(Date)=YEAR(NOW())");
$data=mysqli_fetch_assoc($RESULT);
echo "<br><BR>Number of Students Attended In This Year&nbsp:&nbsp";
echo $data['count(Attendance)'];
$RESULT=mysqli_query($connect, "SELECT count(Placed) from placed_students where Placed=1 AND YEAR(Date)=YEAR(NOW())");
$data=mysqli_fetch_assoc($RESULT);
